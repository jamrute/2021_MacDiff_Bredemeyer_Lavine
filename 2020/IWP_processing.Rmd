...
```{r}
data_dir <- "/Users/jamrute/Documents/Graduate_School/Thesis_Lab/Lavine_Projects/Converted_to_v3/IWP/filtered_feature_bc_matrix/"
sample.data <- Read10X(data.dir =data_dir)
sample <- CreateSeuratObject(counts = sample.data, project = "IWP", min.cells = 3, min.features = 200)
```
...
sample <- FindClusters(sample, resolution = c(0.3, 0.4, 0.8, 0.6, 0.7, 0.5), verbose = FALSE)
...
# Set the Desired resolution as active.ident
```{r}
sample@active.ident <- sample@meta.data$SCT_snn_res.0.5
sample@meta.data$seurat_clusters <- sample@meta.data$SCT_snn_res.0.5
```
...
```{r}
new.cluster.ids <- c("RBCs", "1", "Mast", "Megak", "4", "Macrophage", "Monocyte", "7", "Hemogenic Endothelium" ,"9", "10")
...
# Save a .rds
saveRDS(sample, file = file.path(save_dir, "IWP.rds"))

# Save the normalized SCT matrix
write.csv(as.matrix(sample[["SCT"]]@scale.data), 
          file = file.path(save_dir, "IWP_SCT_normalized.txt"), quote = FALSE)

# Save the meta data
write.csv(sample@meta.data, file = file.path(save_dir, "IWP_meta.csv"), quote = FALSE)

# Save the top DGE for clusters
write.csv(sample.markers, file = file.path(save_dir, "IWP_DGE.csv"), quote = FALSE)

# Save a loom version for loading into scanpy
sample.loom <- as.loom(sample, filename = file.path(save_dir, "IWP.loom"), verbose = FALSE)
sample.loom$close_all()
```


```{r}
sample <- readRDS("./IWP.rds")
```
...
write.csv(sample.markers, file = file.path("/Users/jamrute/Desktop", "2020_IWP_DGE.csv"), quote = FALSE)
```
